...
if [[ "$NIX_BUILD" = true ]]; then
  ...
  if [[ -z "$nixExec" ]]; then
    ...
    echo "Recommended: install Determinate Nix for MacOS: https://docs.determinate.systems/"
    exit 1
  fi
  ...
  echo "Skipping Nix build."
fi

appContentsDir="$projDir/result/Applications/IINA Advance.app/Contents"
...
if [[ "$REPLACE_LIBS" = true ]]; then
  ...
  for srclib in "$srcLibDir/"*
  do
    if [[ "$srclib" == *".dylib" ]]; then
      cp -v "$srclib" "$dstLibDir/"
    fi
  done
else
  echo "Skipping replace deps/lib."
fi
...
if [[ "$REPLACE_EXECUTABLES" = true ]]; then
  ...
  echo "Skipping: replace dep/executable."
fi
...
if [[ "$REPLACE_INCLUDES" = true ]]; then
  ...
  echo "Skipping: replace deps/include."
fi
