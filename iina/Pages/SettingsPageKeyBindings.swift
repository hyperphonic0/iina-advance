...
fileprivate class ConfigEditor: SettingsAccessory.Base {
  ...
  var configNames: [String] {
    let state = ConfTableState.current
    return Constants.InputConf.defaults.map { $0.key } + Array(state.userConfDict.keys).sorted()
  }
  ...
  let configNameValidator: Utility.InputValidator<String> = { input in
    ...
    if state.userConfDict[input] != nil || Constants.InputConf.defaults[input] != nil {
...
// TODO:   PlayerCore.setKeyBindings(mappingController.arrangedObjects as! [KeyMapping])
  }

  @MainActor
  private func getFilePath(forConfig conf: String, showAlert: Bool = true) -> String? {
    let state = ConfTableState.current
    let path = Constants.InputConf.defaults[conf] ?? state.userConfDict[conf]
    if path == nil {
      if showAlert {
