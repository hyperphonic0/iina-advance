...
final class InputConfFileCache {
  static let fileDQ = DispatchQueue.newDQ(label: "com.iina_advance.InputConfFile-FileSystemQueue", qos: .utility)
...
fileprivate func loadFile(at filePath: String, isReadOnly: Bool = true, confName: String? = nil) -> InputConfFile {
  ...
  while let rawLine: String = reader.nextLine() {
    guard lines.count < Constants.maxConfFileLinesAccepted else {
      Logger.log("Maximum number of lines (\(Constants.maxConfFileLinesAccepted)) exceeded: stopping load of file: \(filePath.pii.quoted)")
...
struct InputConfFile: Sendable {
  ...
  func overwriteFile(with newMappings: [KeyMapping]) -> InputConfFile {
    ...
    do {
      ...
      // Should be no danger for `currentInputConfigName` race: none of the actions which overwrite the file can change the current conf
      if Preference.string(for: .currentInputConfigName) == confName {
